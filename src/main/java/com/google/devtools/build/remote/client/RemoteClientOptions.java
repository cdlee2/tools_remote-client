...
public final class RemoteClientOptions {
  ...
  public static class PrintLogCommand {
    ...
    @Parameter(
      ...
      description =
          "Only print log entries for failures (non-zero exit status, excluding cache "
              + "miss induced NOT_FOUND status). "
    )
    ...
    @Parameter(
      names = {"--stats_only", "-s"},
      description = "Only print log summary statistics (number of calls, actions, etc)."
    )
