...
public class LogPrintingUtils {
  ...
  /**
   * Returns whether the call in the entry exited with a failed status. NOT_FOUND errors caused by
   * action cache miss are not counted as failures.
   */
  private static boolean isFailedCall(LogEntry entry) {
    if (entry.getStatus().getCode() == Code.OK.value()) {
      ...
      // Do not count cache misses as a failed call.
      return entry.getStatus().getCode() != Code.NOT_FOUND.value();
    }
    return true;
  }

  /**
   * Returns true only if the log entry should be printed according to the given command options.
   */
  private static boolean filterLogEntry(LogEntry entry, PrintLogCommand options) {
    if (options.failuresOnly && isFailedCall(entry)) {
      return false;
    }
